    ...
    while true do
        ...
        if not display and nearJailingUi then
            ...
            if IsControlJustPressed(0, 51) then
                TriggerServerEvent("Ayse_Jailing:getPlayers")
            end
        end
    end
end)

RegisterNetEvent("Ayse_Jailing:returnPlayers")
AddEventHandler("Ayse_Jailing:returnPlayers", function(players)
...
AddEventHandler("playerSpawned", function()
    TriggerServerEvent("Ayse_Jailing:getJailTime")
end)
...
function jail(time)
    ...
    TriggerEvent("Ayse_Jailing:timeLeft")
    TriggerServerEvent("Ayse_Jailing:updateJailing", jailTime)
...
RegisterNetEvent("Ayse_Jailing:timeLeft")
AddEventHandler("Ayse_Jailing:timeLeft", function()
    while jailTime > 0 do
        Citizen.Wait(0)
...
AddEventHandler("Ayse_Jailing:jailPlayer", function(time)
    jail(time)
end)

RegisterNetEvent("Ayse_Jailing:unjailPlayer")
AddEventHandler("Ayse_Jailing:unjailPlayer", function(time)
    jailTime = 0
end)
...
RegisterCommand("jail", function(source, args, rawCommand)
    ...
    if hasAccess() then
        ...
        elseif time <= config.maxJailTime then
            TriggerServerEvent("Ayse_Jailing:jailPlayer", id, time, fine, reason)
            SetDisplay(false)
        end
    else
        TriggerEvent("chat:addMessage", {
